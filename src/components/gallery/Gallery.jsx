...
export default function GalleryGrid() {
  ...
  return (
    <section className="relative bg-[#0A0A0A] text-[#F8F6F2] font-poppins py-24 lg:py-32 overflow-hidden border-t border-white/[0.05]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Dynamic Filtering Navigation Bar */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16 pb-6 border-b border-white/[0.05]">
          <div className="flex items-center space-x-3">
            <Layers className="w-4 h-4 text-[#C8A96A]" />
            <span className="font-cinzel text-xs tracking-widest text-[#C8A96A] uppercase font-bold">Event Gallery</span>
          </div>
              ...
              <button
                ...
                className={`px-4 py-2 text-xs uppercase tracking-widest transition-all duration-300 ${
                  activeFilter === filter.id 
                    ...
                    : 'bg-white/[0.02] text-gray-400 border border-white/[0.05] hover:border-white/20'
                }`}
              >
                {filter.label}
              </button>
          ...
          <AnimatePresence mode="popLayout">
            {filteredItems.map((item) => (
              <motion.div
                ...
                className={`relative overflow-hidden border border-white/[0.05] bg-[#111] group cursor-pointer ${item.span}`}
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent opacity-80 group-hover:opacity-90 transition-opacity duration-300 z-10" />
                ...
                <div className="absolute inset-0 z-20 p-6 flex flex-col justify-between transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                  <div className="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="w-8 h-8 bg-black/60 border border-white/20 flex items-center justify-center backdrop-blur-sm">
                      <Maximize2 className="w-3.5 h-3.5 text-[#C8A96A]" />
                    </div>
                  </div>

                  <div>
                    <span className="font-cinzel text-[9px] tracking-widest text-[#C8A96A] uppercase font-bold block mb-1">
                      {item.location}
                    </span>
                    <h3 className="font-cinzel text-lg sm:text-xl font-medium tracking-wide text-white">
                      {item.title}
                    </h3>
                  </div>
                </div>
        ...
        {selectedMedia && (
          ...
          >
            ...
            <motion.div 
              ...
              className="relative max-w-5xl w-full bg-[#0A0A0A] border border-white/10 z-10 flex flex-col"
            >
              <button 
                onClick={() => setSelectedMedia(null)}
                className="absolute -top-12 right-0 md:top-4 md:right-4 text-gray-400 hover:text-white transition-colors bg-white/5 md:bg-black/40 p-2 border border-white/10 rounded-full z-30"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="p-6 bg-[#111] border-t border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <span className="font-cinzel text-[10px] tracking-widest text-[#C8A96A] uppercase font-bold block mb-1">
                    {selectedMedia.location}
                  </span>
                  <h4 className="font-cinzel text-xl font-medium tracking-wide text-white">
                    {selectedMedia.title}
                  </h4>
                </div>
                <a href='/contact' className="px-5 py-3 bg-[#C8A96A] text-black font-semibold uppercase text-xs tracking-widest hover:bg-[#A67C3D] transition-colors duration-300">
                  Book This Venue
                </a>
              </div>
            </motion.div>
