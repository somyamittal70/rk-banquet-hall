...
export default function FloatingSidePanel() {
  return (
    ...
    */
    <div className="fixed z-50 bottom-6 left-1/2 -translate-x-1/2 md:bottom-auto md:left-auto md:translate-x-0 md:right-0 md:top-2/3 md:-translate-y-1/2 flex flex-row md:flex-col items-center md:items-end gap-3.5 w-fit px-4 py-3  md:bg-transparent md:border-none   md:p-0">
      {actions.map(({ id, icon: Icon, label, href, bg, hoverBg, textColor }) => (
        <a
          ...
          className={`group flex items-center overflow-hidden   shadow-xl shadow-black/20 transition-all duration-300 ${bg} ${hoverBg}`}
        >
          {/* Icon Area */}
          <span className="flex items-center justify-center w-10 h-10 sm:w-10 sm:h-10 shrink-0">
